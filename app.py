...
if OPTION_MENU_AVAILABLE:
    ...
    selected = st.selectbox("Navigation", ["💬 Chat", "📊 Analytics", "🔧 Settings"])
    ...
    if uploaded_files:
        ...
        if st.button("🔄 Process Documents", type="primary"):
            if LANGCHAIN_AVAILABLE:
                process_documents(uploaded_files, api_key, chunk_size, chunk_overlap, model_choice, temperature)
            else:
                st.error("Advanced RAG features require additional dependencies. Using basic chat mode.")
...
def process_documents(uploaded_files, api_key, chunk_size, chunk_overlap, model_choice, temperature):
...
if selected == "💬 Chat":
    # Chat interface
    if st.session_state.conversation_chain is None and LANGCHAIN_AVAILABLE:
        st.markdown("""
    ...
    if prompt := st.chat_input("Ask sophisticated questions about your documents..."):
        ...
        with st.spinner("🧠 Processing with advanced AI..."):
            if api_key and st.session_state.conversation_chain and LANGCHAIN_AVAILABLE:
                try:
                ...
                try:
                    ...
                    # Include context about uploaded files if present
                    context = ""
                    if uploaded_files:
                        context = f"\n\nNote: The user has uploaded {len(uploaded_files)} PDF file(s): {', '.join([f.name for f in uploaded_files])}"
    ...
    if st.session_state.messages:
        ...
        # Response time chart (if plotly is available)
        try:
    ...
    with col2:
        ...
        st.info(f"LangChain Available: {'✅' if LANGCHAIN_AVAILABLE else '❌'}")
